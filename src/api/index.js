...
export const fetchWeather = (lat, lon) => {
  return async (dispatch) => {
    const response = await axios.get(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&lang=vi&appid=${API_KEY}`
    );
...
export const fetchLocation = (city) => {
  return async (dispatch, getState) => {
    await axios
      .get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&lang=vi&appid=${API_KEY}`)
      .then((response) => {
        dispatch({
          type: 'FETCH_LOCATION',
          payload: response.data,
